...
static void sha256(const unsigned int *pass, int pass_len, unsigned int* hash) {
  int plen=pass_len/4;
...
void ripemd160_process( RIPEMD160_CTX *ctx, const uint8_t *data )
{
...
__kernel void generate_btc_addresses(
    ...
    // For 3 unknown words (positions 21, 22 and 23):
    ...
    // Total: 2048 × 2048 × 8 = 33,554,432 combinations
    
    uint last_3_bits = (uint)(current_offset % 8UL);   // 0-7
    ulong temp = current_offset / 8UL;
    uint w22_idx = (uint)(temp % 2048UL);              // 0-2047
    uint w21_idx = (uint)((temp / 2048UL) % 2048UL);   // 0-2047

    // Build word indices (21 known + 2 unknown + 1 checksum)
    // Seed: protect arctic pudding cabbage fiction hub extend board yard december 
    //       service drip suffer fox error note mother online shield stomach engage ??? ??? ???
